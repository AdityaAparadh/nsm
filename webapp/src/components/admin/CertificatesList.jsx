...
const CertificatesList = () => {
  ...
  const fetchCertificates = async () => {
    ...
    try {
      ...
      const response = await certificatesAPI.listCertificates({
        ...
        <table className="table">
          ...
          <tbody>
            {certificates.map((certificate) => (
              <tr key={certificate.id} className="hover">
                <td>{certificate.id}</td>
                <td>
                  <div className="font-bold">{certificate.participant?.fullName || 'N/A'}</div>
                </td>
                <td>
                  <div className="font-medium">{certificate.workshop?.name || 'N/A'}</div>
                </td>
                <td>
                  <code className="text-xs bg-base-200 px-2 py-1 rounded">{certificate.uuid}</code>
                </td>
